		...
		<view class="box-content">
			...
			<view class="box-content-order-info">
				...
				<view class="box-content-order-info-main">
					...
					<view class="box-content-order-info-main-item" v-if="orderInfo.content">
						<view class="box-content-order-info-main-item-title">备注：</view>
						<view class="box-content-order-info-main-item-text">
		...
		methods: {
			...
			getOrderDetails(id) {
				this.apiget('api/v1/engineer/order/' + id).then(res => {
					if (res.status == 200) {
						this.orderInfo = res.data.member
					} else {
						uni.showToast({
							title: res.massage,
							icon: 'none'
